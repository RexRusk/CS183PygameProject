...
class Info:
    ...
    def create_state_lables(self):
        ...
        if self.state == 'main_menu':
            self.state_lables.append((self.create_lable('BRIEF MARIO', int(C.SCREEN_W / 16), (0, 0, 0)), C.SCREEN_W / 2, C.SCREEN_H / 4))
            ...
            self.state_lables.append((self.create_lable('BG Music', int(C.SCREEN_W / 24)), C.SCREEN_W / 2, C.SCREEN_H / 8 * 3))
            self.state_lables.append((self.create_lable('Back', int(C.SCREEN_W / 24)), C.SCREEN_W / 8, C.SCREEN_H * 7 / 8))
        elif self.state == 'level':
            if self.game_info['level'] == 0:
                self.state_lables.append((self.create_lable('SCORE      {}'.format(self.game_info['score']), int(C.SCREEN_W / 36), (0, 0, 0)), C.SCREEN_W / 8, C.SCREEN_H / 8))
            else:
                self.state_lables.append((self.create_lable('SCORE      {}'.format(self.game_info['next_score']), int(C.SCREEN_W / 36), (0, 0, 0)), C.SCREEN_W / 8, C.SCREEN_H / 8))
            self.state_lables.append((self.create_lable('COIN      {}'.format(self.game_info['coin']), int(C.SCREEN_W / 36), (0, 0, 0)), C.SCREEN_W / 8 * 3, C.SCREEN_H / 8))
            self.state_lables.append((self.create_lable('LIVES      {}'.format(self.game_info['lives']), int(C.SCREEN_W / 36), (0, 0, 0)),
                                      C.SCREEN_W / 8 * 5, C.SCREEN_H / 8))
            ...
            if died == 0:
                ...
                self.state_lables.append((self.create_lable('TIME      {}'.format(self.game_info['time']), int(C.SCREEN_W / 36), (0, 0, 0)),
                                          C.SCREEN_W / 8 * 3, C.SCREEN_H / 16 * 3))
            else:
                timer = pygame.time.get_ticks() - time
                self.game_info['time'] = int(timer / 1000)
                self.state_lables.append(
                    (self.create_lable('TIME      {}'.format(self.game_info['time']), int(C.SCREEN_W / 36), (0, 0, 0)),
                     C.SCREEN_W / 8 * 3, C.SCREEN_H / 16 * 3))

            self.state_lables.append((self.create_lable('WORLD      {}'.format(self.game_info['level'] + 1),  int(C.SCREEN_W / 36), (0, 0, 0)),
...
#draw...
    def draw(self, surface):
        ...
        for lable in self.state_lables:
            ...
            surface.blit(lable[0], (lable[1] - lable[0].get_rect().size[0]*0.5, lable[2]-lable[0].get_rect().size[1]*0.5))

        for lable2 in self.info_lables:
            surface.blit(lable2[0], (lable2[1] - lable2[0].get_rect().size[0]*0.5, lable2[2]-lable2[0].get_rect().size[1]*0.5))
